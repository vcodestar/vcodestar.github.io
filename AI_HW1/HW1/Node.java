...
public class Node implements Comparable<Node>
{
    ...
    public Node(ArrayList<ArrayList<Cube>> state, int k, float cost, int depth)
    {
        this.k = k;
        this.state =state;
        this.cost = cost;
    ...
    public ArrayList<Node> getNextStates()
    {
                    ...
                    for(int y = 1; y <= 3 ; y++)
                    {
                        for(int x = 1; x <= state.get(y - 1).size(); x++)
                        {
                            if(isValid(cube, x, y))
                            {
                                ...
                                Node child = new Node(childState, k, cost, depth + 1);
                                int cubeX = cube.getX();
                                int cubeY = cube.getY();
    ...
    void RemoveCube(int cubeX, int cubeY)
    {
